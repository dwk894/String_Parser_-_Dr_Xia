...
public class StringChecker {
    ...
    public StringChecker(String originalString) {
        ...
        // If "original" does not have ';' at the end, add a ';' at the end before
        // further checking.
        if (!original.replaceAll("\\s+", "").isEmpty() && original.charAt(original.length() - 1) != ';') {
            original += ';';
        }
        ...
        while ((!original.isEmpty()) && (original.charAt(0) < 48 || original.charAt(0) > 57)) {
    ...
    private static boolean isValidTime(String expression) {
        ...
        if (!isValidHour(expression.split(":")[0])) {
            return false;
        }
        if (!isValidMinute(expression.split(":")[1])) {
            return false;
        }
        return true;
    }
    ...
    private static boolean isValidPeriod(String expression) {
        /**
         * Check if the "expression" is a valid time period ("hh:mm-hh:mm" format). The
         * second time (after hyphen) must be later than the first time (before hyphen).
         * 
         * @return : true (valid) or false (invalid)
         */
        if (count(expression, '-') != 1) {
        ...
        for (int i = 0; i < tokens.length; i++) {
            if (i == 0) { // The first token is the switch number, which is a special token.
                if (!isAllDigits(tokens[i])
                        || (!tokens[i].equals("1") && !tokens[i].equals("2") && !tokens[i].equals("3"))) {
                    return false;
                }
            } else {
                ...
                if (!isAllDigits(bright) || Integer.parseInt(bright) < 0 || Integer.parseInt(bright) > MAX_BRIGHTNESS) {
                    return false;
                }
                ...
                if (previousEndHour != null && previousEndMinute != null) {
                    ...
                    int currentMinute = Integer
                            .parseInt(timePeriod.substring(timePeriod.indexOf(':') + 1, timePeriod.indexOf('-')));
